...
model TLM "Constant Parameter (CP) line/cable model (multiphase)"
  ...
  //Final Paramters
  final parameter Real R[m]=r*d/1000 annotation(HideResult=true);
  final parameter Real h[m]=(Zc.-R./4)./(Zc.+R./4) annotation(HideResult=true);
  final parameter Real Zmod[m]=(Zc.+R./4) annotation(HideResult=true);
  final parameter Real RN[m,m]=Modelica.Math.Matrices.inv(transpose(Ti)) * diagonal(Zmod) * Modelica.Math.Matrices.inv(Ti) annotation(HideResult=true);
